...
#else
            ((Indices >= sizeof(_storage) / 16 ? static_cast<void>(0) : _mm_storeu_si128(
                reinterpret_cast<__m128i*>(_storage) + Indices,
                _mm_xor_si128(_mm_loadu_si128(reinterpret_cast<const __m128i*>(_storage) + Indices),
                    _mm_loadu_si128(reinterpret_cast<const __m128i*>(keys) + Indices)))), ...);
#endif
        }
